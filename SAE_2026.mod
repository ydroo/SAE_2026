...
dvar boolean x[Nodes][Nodes][Vehicles]; // 1 si véhicule v passe de i à j
dvar int+ Q[Nodes][Vehicles];           // charge transportée après i
dvar boolean y[Vehicles];               // 1 si le véhicule est utilisé
...
subject to {
  ...
  nbrVehicules == sum(v in Vehicles) y[v];
}
